...
const CounselorDashboard = () => {
  ...
  useEffect(() => {
    Promise.all([getDashboardStats(), getAlerts(), getCounselorUsers()])
      .then(([s, a, u]) => { setStats(s.data.stats); setAlerts(a.data.alerts); setUsers(u.data.users); })
      .catch(console.error)
  ...
  const handleSearch = async () => {
    try {
      const res = await getCounselorUsers(search, riskFilter);
      setUsers(res.data.users);
    } catch (err) { console.error(err); }
  };

  useEffect(() => { handleSearch(); }, [search, riskFilter]);

  const StatCard = ({ icon, value, label, color }) => (
    <div className="relative overflow-hidden p-4 bg-white dark:bg-ink-900 rounded-2xl border border-ink-100 dark:border-ink-800 shadow-sm">
      <div className={`absolute top-0 right-0 w-16 h-16 rounded-full -mr-4 -mt-4 opacity-10 ${color}`} />
      <div className="text-2xl mb-1">{icon}</div>
      <div className="text-2xl font-bold text-ink-900 dark:text-white" style={{fontFamily:'Sora,sans-serif'}}>{value ?? '—'}</div>
  ...
  return (
    <div className="min-h-screen bg-ink-50 dark:bg-ink-950 transition-colors duration-300">
        ...
        <div className="flex items-center justify-between max-w-6xl px-4 py-3 mx-auto sm:px-6">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />Counselor
            </span>
            {stats?.unresolved_alerts > 0 && (
              <span className="flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-500 rounded-full">{stats.unresolved_alerts}</span>
            )}
          </div>
      ...
      <main className="max-w-6xl px-4 py-6 sm:py-8 mx-auto sm:px-6">
        <div className="mb-6">
          <div className="w-8 h-1 mb-3 rounded-full bg-blue-500" />
          <h1 className="text-2xl sm:text-3xl font-bold text-ink-900 dark:text-white" style={{fontFamily:'Sora,sans-serif'}}>Counselor Dashboard</h1>
          ...
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-3 sm:px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                tab === t.id ? 'bg-white dark:bg-ink-900 text-ink-900 dark:text-white shadow-sm' : 'text-ink-500 dark:text-ink-400 hover:text-ink-700'
              }`}>
              {t.label}
              <span className={`ml-1.5 px-1.5 py-0.5 rounded-full text-xs ${tab === t.id ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400' : 'bg-ink-200 dark:bg-ink-700 text-ink-500'}`}>
                {t.count}
              </span>
            </button>
          ))}
        ...
        {tab === 'alerts' && (
          <div>
            <h2 className="mb-4 font-semibold text-ink-900 dark:text-white">
              Unresolved Alerts {alerts.length > 0 && <span className="text-sm font-normal text-ink-400">({alerts.length} pending)</span>}
            </h2>
            {loading ? (
              <div className="flex items-center justify-center py-16 text-ink-400">
                <svg className="w-5 h-5 mr-2 animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 11-6.219-8.56"/></svg>Loading...
              </div>
            ) : alerts.length === 0 ? (
              ...
              <div className="space-y-3">
                {alerts.map(alert => (
                  <div key={alert.id} className={`bg-white dark:bg-ink-900 rounded-2xl border shadow-sm overflow-hidden ${alert.risk_level === 'High' ? 'border-red-200 dark:border-red-900' : 'border-ink-100 dark:border-ink-800'}`}>
                    {alert.risk_level === 'High' && <div className="h-1 bg-gradient-to-r from-red-500 to-orange-500" />}
                    <div className="p-4 sm:p-5">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center justify-center w-10 h-10 text-sm font-bold rounded-xl bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-300 flex-shrink-0">
                      ...
                      <div className="grid grid-cols-3 gap-2 p-3 mb-3 rounded-xl bg-ink-50 dark:bg-ink-800">
                        {[
                          { label: 'Mood', value: `${moodEmoji(alert.mood_score)} ${alert.mood_score}/10` },
                          { label: 'Stress', value: `${alert.stress_level || '—'}/10` },
                          { label: 'Anxiety', value: `${alert.anxiety_level || '—'}/10` },
                        ].map(s => (
                          <div key={s.label} className="text-center">
                            <p className="text-sm font-bold text-ink-900 dark:text-white">{s.value}</p>
                      ...
                      {alert.summary && <p className="mb-3 text-sm leading-relaxed text-ink-600 dark:text-ink-300">{alert.summary}</p>}
                      ...
                      {resolveId === alert.id && (
                        <div className="mb-3">
                          <textarea value={resolveNote} onChange={e => setResolveNote(e.target.value)}
                            ...
                            rows={2}
                            className="w-full px-3 py-2 text-sm border resize-none rounded-xl border-ink-200 dark:border-ink-700 bg-ink-50 dark:bg-ink-800 text-ink-900 dark:text-white placeholder-ink-400 focus:outline-none focus:ring-2 focus:ring-verde-500" />
                        </div>
                      )}

                      <div className="flex items-center justify-between pt-3 border-t border-ink-50 dark:border-ink-800">
                        <span className="text-xs text-ink-400">{new Date(alert.created_at).toLocaleString()}</span>
                        <div className="flex items-center gap-2">
                          ...
                          {resolveId === alert.id ? (
                            <>
                              <button onClick={() => { setResolveId(null); setResolveNote(''); }}
                                className="px-3 py-1.5 text-xs font-medium border rounded-lg text-ink-500 border-ink-200 dark:border-ink-700 hover:bg-ink-50 transition-all">
                                Cancel
                              </button>
                              <button onClick={() => handleResolve(alert.id)} disabled={resolving === alert.id}
                                ...
                                {resolving === alert.id ? 'Resolving...' : '✓ Confirm'}
                              </button>
                            </>
                          ) : (
                            <button onClick={() => setResolveId(alert.id)}
                              className="px-3 py-1.5 text-xs font-medium text-white transition-all rounded-lg bg-verde-600 hover:bg-verde-700">
                              ✓ Resolve
                            </button>
                          )}
                        </div>
        ...
        {tab === 'users' && (
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              <div className="relative flex-1">
                ...
                <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                  ...
                  className="w-full pl-9 pr-4 py-2.5 text-sm border rounded-xl border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 text-ink-900 dark:text-white placeholder-ink-400 focus:outline-none focus:ring-2 focus:ring-verde-500" />
              </div>
              <select value={riskFilter} onChange={e => setRiskFilter(e.target.value)}
                className="px-4 py-2.5 text-sm border rounded-xl border-ink-200 dark:border-ink-700 bg-white dark:bg-ink-900 text-ink-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-verde-500">
            ...
            <h2 className="mb-4 font-semibold text-ink-900 dark:text-white">
              Users <span className="text-sm font-normal text-ink-400">({users.length} found)</span>
            </h2>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 bg-white border border-dashed dark:bg-ink-900 rounded-2xl border-ink-200 dark:border-ink-700">
                <div className="mb-3 text-5xl">👥</div>
                <p className="font-medium text-ink-500 dark:text-ink-400">No users found</p>
              </div>
            ) : (
              <>
                {/* Desktop Table */}
                <div className="hidden sm:block overflow-x-auto bg-white dark:bg-ink-900 rounded-2xl border border-ink-100 dark:border-ink-800 shadow-sm">
                      ...
                      {users.map(u => (
                        <tr key={u.id} className="hover:bg-ink-50/50 dark:hover:bg-ink-800/30 transition-colors">
                          <td className="px-5 py-4">
                            <div className="flex items-center gap-3">
                              <div className="flex items-center justify-center w-8 h-8 text-sm font-semibold rounded-full bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-300 flex-shrink-0">
                          ...
                          <td className="px-5 py-4">
                            <span className="inline-flex items-center justify-center w-7 h-7 text-xs font-semibold rounded-full bg-verde-50 dark:bg-verde-950/30 text-verde-700 dark:text-verde-400">{u.total_logs}</span>
                          </td>
                          <td className="px-5 py-4">
                  ...
                  {users.map(u => (
                    <div key={u.id} className="p-4 bg-white border shadow-sm dark:bg-ink-900 rounded-2xl border-ink-100 dark:border-ink-800">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="flex items-center justify-center w-9 h-9 text-sm font-semibold rounded-full bg-ink-100 dark:bg-ink-700 text-ink-600 dark:text-ink-300 flex-shrink-0">
                            {u.full_name?.charAt(0).toUpperCase()}
                          </div>
                          <div>
                      ...
                      <div className="grid grid-cols-3 gap-2 pt-3 text-center border-t border-ink-50 dark:border-ink-800 mb-3">
                        <div>
                      ...
                      <button onClick={() => handleViewUser(u)}
                        className="w-full py-2 text-xs font-medium border rounded-xl text-ink-600 dark:text-ink-300 border-ink-200 dark:border-ink-700 hover:bg-ink-50 dark:hover:bg-ink-800 transition-all">
                        View Mood History
                      </button>
                    </div>
                  ))}
        ...
        {tab === 'userDetail' && selectedUser && (
          <div>
            ...
            {userLogs.length > 0 && <div className="mb-4"><UserMoodChart logs={userLogs} /></div>}
            ...
            {userLogs.length === 0 ? (
              ...
              <div className="space-y-3">
                {userLogs.map(log => (
                  <div key={log.id} className="p-4 sm:p-5 bg-white border shadow-sm dark:bg-ink-900 rounded-2xl border-ink-100 dark:border-ink-800">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 p-3 mb-3 text-center rounded-xl bg-ink-50 dark:bg-ink-800">
                      <div><p className="text-sm font-semibold text-ink-900 dark:text-white">{log.sleep_hours || '—'}h</p><p className="text-xs text-ink-400">Sleep</p></div>
                      <div><p className="text-sm font-semibold text-ink-900 dark:text-white">{log.stress_level || '—'}/10</p><p className="text-xs text-ink-400">Stress</p></div>
                      <div><p className="text-sm font-semibold text-ink-900 dark:text-white">{log.anxiety_level || '—'}/10</p><p className="text-xs text-ink-400">Anxiety</p></div>
                    </div>
                    {log.symptoms && (
                      <div className="flex flex-wrap gap-1.5 mb-3">
                    ...
                    {log.summary && <p className="text-sm leading-relaxed text-ink-500 dark:text-ink-400">{log.summary}</p>}
                  </div>
                ))}
              </div>
            )}
